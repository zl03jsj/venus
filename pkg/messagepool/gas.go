...
func (mp *MessagePool) GasEstimateMessageGas(ctx context.Context, estimateMessage *types.EstimateMessage, _ types.TipSetKey) (*types.Message, error) {
	...
	if estimateMessage.Msg.GasLimit == 0 {
		...
		estimateMessage.Msg.GasLimit = gasLimit
		if estimateMessage.Spec.GasOverEstimation > 1 {
			estimateMessage.Msg.GasLimit = int64(float64(estimateMessage.Msg.GasLimit) * estimateMessage.Spec.GasOverEstimation)
		}
	}
